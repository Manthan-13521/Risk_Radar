...
export default async function InvestigationResult({ params }: { params: { id: string } }) {
  ...
  // Generate deterministic voice summary server-side — no extra AI call
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className={`p-6 rounded-lg border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${
          ...
          <div className="flex gap-4">
            <div className="text-center bg-zinc-950/50 px-4 py-2 rounded border border-zinc-800">
              ...
              <div className="text-xs text-zinc-600 mt-0.5">How dangerous the behavior appears</div>
              <div className="text-2xl font-bold mt-1">{scan.riskScore} <span className="text-sm text-zinc-500">/ 100</span></div>
            </div>
            <div className="text-center bg-zinc-950/50 px-4 py-2 rounded border border-zinc-800">
              <div className="text-xs text-zinc-500 uppercase font-semibold">Confidence</div>
              <div className="text-xs text-zinc-600 mt-0.5">How strong the available evidence is</div>
              <div className="text-2xl font-bold mt-1">{scan.confidenceScore} <span className="text-sm text-zinc-500">/ 100</span></div>
            </div>
          </div>
        </div>

        {/* Voice Verdict — automatic, async, non-blocking */}
        <VoiceVerdict
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
          <div>
            ...
            <p className="text-xs text-zinc-600 mt-1">Simulated action — does not modify your system</p>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              ...
              <div className="col-span-2">
                ...
                <div className="font-mono text-sm truncate text-zinc-400 group relative">
                  {scan.inputMetadata.sha256 ? (scan.inputMetadata.sha256 as string).substring(0, 16) + '...' : 'N/A'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="col-span-1 bg-zinc-900 p-5 rounded-lg border border-zinc-800">
            ...
            <p className="text-xl font-medium capitalize text-blue-200">{(scan.attackerIntent as string).replace(/_/g, ' ')}</p>
          </div>
        ...
        {/* DNA Match */}
        {scan.dnaOverlap && (scan.dnaOverlap as Array<{overlapPercent: number; previousIntent: string; sharedTags: string[]}>).length > 0 && (scan.dnaOverlap as Array<{overlapPercent: number}>)[0].overlapPercent >= 50 ? (
          <div className="bg-blue-950/30 border border-blue-900/50 p-6 rounded-lg relative overflow-hidden">
            ...
            <p className="text-blue-300 font-medium mb-4">
              {(scan.dnaOverlap as Array<{overlapPercent: number}>)[0].overlapPercent}% behavioral similarity with a previously detected threat.
            </p>
            <div className="flex flex-col gap-2 bg-zinc-950/50 p-4 rounded border border-blue-900/30">
              <div className="text-sm">
                ...
                <span className="capitalize font-medium">{((scan.dnaOverlap as Array<{previousIntent: string}>)[0].previousIntent).replace(/_/g, ' ')}</span>
              </div>
              <div className="text-sm text-zinc-400 mb-1">Shared Behavioral Signals:</div>
              <div className="flex flex-wrap gap-2">
                {((scan.dnaOverlap as Array<{sharedTags: string[]}>)[0].sharedTags).map((tag: string, i: number) => (
        ...
        <div className="space-y-3">
          ...
          {scan.evidence && (scan.evidence as Array<Record<string, string>>).map((e, i: number) => (
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {scan.dnaTags && (scan.dnaTags as string[]).length > 0 ? (scan.dnaTags as string[]).map((tag: string, i: number) => (
