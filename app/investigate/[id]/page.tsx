...
const VERDICT_CONFIG = {
  safe:      { label: 'NO SIGNIFICANT THREAT',  accent: '#176B52', bg: 'rgba(23,107,82,0.08)',  border: 'rgba(23,107,82,0.3)' },
  suspicious:{ label: 'SUSPICIOUS ACTIVITY',    accent: '#B86A00', bg: 'rgba(184,106,0,0.08)', border: 'rgba(184,106,0,0.3)' },
  dangerous: { label: 'THREAT DETECTED',         accent: '#990011', bg: 'rgba(153,0,17,0.08)',  border: 'rgba(153,0,17,0.3)' },
...
export default async function InvestigationResult({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1100px] mx-auto space-y-8" style={{ background: '#ECE6E2' }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-mono font-medium" style={{ color: '#554B49' }}>ID: {params.id.substring(0, 8)}…</span>
      </div>

      {/* HERO VERDICT */}
      <div className="rounded-2xl border p-8 shadow-sm" style={{ background: v.bg, borderColor: v.border }}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-2">
            <div className="text-[10px] font-extrabold uppercase tracking-widest" style={{ color: v.accent }}>Risk_Radar Verdict</div>
            <h1 className="text-3xl md:text-4xl font-extrabold leading-tight" style={{ color: '#111111' }}>{v.label}</h1>
            <div className="text-sm" style={{ color: '#554B49' }}>
              <span className="font-mono font-bold capitalize">{cls} classification</span>
              {' · '}
              <span className="capitalize font-medium">{formatIntent(String(scan.attackerIntent || 'uncertain'))} intent</span>
            </div>
            <div className="text-xs font-mono font-bold" style={{ color: '#554B49' }}>
          ...
          <div className="flex gap-4 shrink-0">
            <div className="text-center px-6 py-4 rounded-xl border shadow-xs" style={{ background: '#ECE6E2', borderColor: '#C4B5B0' }}>
              <div className="text-[10px] font-extrabold uppercase tracking-widest mb-1" style={{ color: '#554B49' }}>RISK</div>
              <div className="text-4xl font-extrabold font-mono" style={{ color: v.accent }}>{Number(scan.riskScore)}</div>
              <div className="text-xs font-medium" style={{ color: '#554B49' }}>/100</div>
              <div className="text-[10px] mt-1" style={{ color: '#554B49' }}>How dangerous this behavior appears.</div>
            </div>
            <div className="text-center px-6 py-4 rounded-xl border shadow-xs" style={{ background: '#ECE6E2', borderColor: '#C4B5B0' }}>
              <div className="text-[10px] font-extrabold uppercase tracking-widest mb-1" style={{ color: '#554B49' }}>CONFIDENCE</div>
              <div className="text-4xl font-extrabold font-mono" style={{ color: '#111111' }}>{Number(scan.confidenceScore)}</div>
              <div className="text-xs font-medium" style={{ color: '#554B49' }}>%</div>
              <div className="text-[10px] mt-1" style={{ color: '#554B49' }}>How strong the evidence is.</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border p-6 shadow-sm" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
        <div className="text-[10px] font-extrabold uppercase tracking-widest mb-3" style={{ color: '#554B49' }}>Attacker Intent</div>
        ...
        <p className="text-sm mt-2 font-medium" style={{ color: '#554B49' }}>
          Likely objective based on behavioral heuristics and AI reasoning analysis.
        </p>
      </div>

      {/* POLICY DECISION */}
      <div className="rounded-2xl border p-6 flex flex-col md:flex-row md:items-center justify-between gap-5 shadow-sm" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
        <div>
          <div className="text-[10px] font-extrabold uppercase tracking-widest mb-2" style={{ color: '#554B49' }}>Recommended Action</div>
        ...
        {scan.actionTaken ? (
          <div className="px-5 py-3 rounded-xl border text-sm font-bold shadow-xs" style={{ background: '#ECE6E2', borderColor: '#C4B5B0', color: '#176B52' }}>
          ...
          <form action={`/api/scans/${scan._id}/action`} method="POST">
            <button
              ...
              className="px-6 py-3 rounded-xl text-sm font-extrabold text-white transition hover:opacity-90 shadow-sm"
      ...
      <div id="evidence-section" className="rounded-2xl border shadow-sm" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
        <div className="px-6 py-5 border-b" style={{ borderColor: '#C4B5B0' }}>
          <div className="text-xs font-extrabold uppercase tracking-widest" style={{ color: '#111111' }}>WHY THIS WAS FLAGGED</div>
        </div>
        <div className="p-6">
          {(!scan.evidence || (scan.evidence as unknown[]).length === 0) ? (
            <div className="text-sm font-medium" style={{ color: '#554B49' }}>No malicious signals identified. Artifact verified clean.</div>
          ) : (
            <div className="space-y-5">
              {(scan.evidence as Array<Record<string, string>>).map((e, i) => {
                const sevColor = e.severity === 'critical' || e.severity === 'high' ? '#990011' : e.severity === 'medium' ? '#B86A00' : '#554B49';
                const sevBg = e.severity === 'critical' || e.severity === 'high'
                  ? 'rgba(153,0,17,0.1)' : e.severity === 'medium' ? 'rgba(184,106,0,0.1)' : 'rgba(111,102,100,0.1)';
                return (
                  <div key={i} className="flex gap-4">
                    <div className="mt-0.5 shrink-0">
                      <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded uppercase" style={{ background: sevBg, color: sevColor }}>{e.severity}</span>
                    </div>
                    <div>
                      <div className="font-bold text-sm" style={{ color: '#111111' }}>{e.title}</div>
                      <div className="text-sm mt-0.5 font-medium" style={{ color: '#554B49' }}>{e.description}</div>
                    </div>
                  </div>
                );
              })}
      ...
      <div className="rounded-2xl border shadow-sm" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
        <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: '#C4B5B0' }}>
          <div>
            <div className="text-xs font-extrabold uppercase tracking-widest" style={{ color: '#111111' }}>THREAT DNA</div>
            <div className="text-xs font-medium" style={{ color: '#554B49' }}>Behavioral memory</div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {scan.dnaTags && (scan.dnaTags as string[]).length > 0 ? (
              (scan.dnaTags as string[]).map((tag, i) => (
                <span key={i} className="px-3 py-1.5 rounded-lg border text-xs font-mono font-bold" style={{ background: '#D3C9C5', borderColor: '#C4B5B0', color: '#111111' }}>{tag}</span>
              ))
            ) : (
              <span className="text-xs italic font-medium" style={{ color: '#554B49' }}>No attack signatures assigned.</span>
            )}
          </div>
          {scan.dnaOverlap && (scan.dnaOverlap as Array<Record<string, unknown>>).length > 0 && (
            <div className="rounded-xl border p-4 flex items-center justify-between shadow-xs" style={{ background: 'rgba(153,0,17,0.06)', borderColor: 'rgba(153,0,17,0.2)' }}>
              <div>
                <div className="text-sm font-extrabold" style={{ color: '#990011' }}>
                  {Number((scan.dnaOverlap as Array<Record<string, unknown>>)[0].overlapPercent)}% BEHAVIORAL SIMILARITY
                </div>
                <div className="text-xs mt-0.5 font-medium" style={{ color: '#554B49' }}>Matches previously observed attack vector.</div>
              </div>
              <Link href="/threat-dna" className="px-3.5 py-1.5 rounded-xl text-xs font-extrabold text-white shadow-xs" style={{ background: '#990011' }}>Compare →</Link>
            </div>
          )}
        </div>
      </div>
