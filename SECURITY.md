...
Risk Radar is an analysis engine designed to evaluate potentially malicious URLs, messages, and files securely. The following principles govern its security architecture.
...
Risk Radar inspects file metadata and extracts text from supported document types (`.pdf`, `.docx`, `.txt`, `.csv`) entirely server-side.
- **No execution:** Risk Radar will *never* execute binaries, scripts, or macros.
...
- Risk Radar does not alter OS settings, firewall rules, or interact with external third-party accounts (e.g., email providers) to block or delete actual user data.
