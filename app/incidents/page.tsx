        ...
        {[
          ...
          <div key={m.label} className="rounded-xl border p-5 shadow-sm" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
            ...
            <div className="text-3xl font-extrabold font-mono" style={{ color: m.color }}>
              {m.value}
              {m.suffix && <span className="text-base font-normal" style={{ color: '#554B49' }}>{m.suffix}</span>}
            </div>
          </div>
        ))}
      </div>
        ...
        {FILTERS.map(f => {
          ...
          return (
            <Link
              ...
              className="px-3.5 py-2 rounded-xl text-xs font-extrabold transition shrink-0 shadow-xs"
              style={{
                background: active ? '#990011' : '#E0D8D4',
                color: active ? '#fff' : '#554B49',
                border: `1px solid ${active ? '#990011' : '#C4B5B0'}`,
              }}
            >
              {f.label}
            </Link>
