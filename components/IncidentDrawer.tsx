  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" role="dialog" aria-modal="true">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        ...
        >
          {/* Header */}
          <div className="p-6 border-b flex items-start justify-between gap-4 shrink-0" style={{ borderColor: '#C4B5B0', background: '#E0D8D4' }}>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span
                  className="px-2.5 py-0.5 rounded text-[10px] font-extrabold uppercase"
                  style={{ background: 'rgba(153,0,17,0.12)', color: sevColor, border: `1px solid ${sevColor}` }}
                >
                  {incident.severity}
                </span>
                <span className="text-xs font-mono font-bold" style={{ color: '#554B49' }}>
                  {incident.incidentId}
                </span>
              </div>
              ...
              <div className="text-[11px] font-medium" style={{ color: '#554B49' }}>
          ...
          <div className="p-6 space-y-6 flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-3 gap-3 p-4 rounded-xl border" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
              <div className="text-center">
                <div className="text-[10px] font-extrabold uppercase tracking-widest" style={{ color: '#554B49' }}>Risk</div>
                <div className="text-2xl font-extrabold font-mono mt-0.5" style={{ color: '#990011' }}>
                  {incident.riskScore}
                  <span className="text-xs font-normal" style={{ color: '#554B49' }}>/100</span>
                </div>
              </div>
              <div className="text-center border-x" style={{ borderColor: '#C4B5B0' }}>
                <div className="text-[10px] font-extrabold uppercase tracking-widest" style={{ color: '#554B49' }}>Confidence</div>
                <div className="text-2xl font-extrabold font-mono mt-0.5" style={{ color: '#111111' }}>
                  {incident.confidenceScore}%
                </div>
              </div>
              <div className="text-center">
                <div className="text-[10px] font-extrabold uppercase tracking-widest" style={{ color: '#554B49' }}>Status</div>
                <div className="text-xs font-extrabold uppercase font-mono mt-2" style={{ color: status === 'resolved' ? '#176B52' : '#990011' }}>
                  {status}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border p-4 space-y-2" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
              ...
              <div className="space-y-1.5 text-xs leading-relaxed" style={{ color: '#111111' }}>
                <div className="flex items-start gap-2">
                  <span className="font-extrabold" style={{ color: '#990011' }}>1.</span>
                  <span className="font-bold">Objective: {formattedIntent || 'UNCONFIRMED THREAT'}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="font-extrabold" style={{ color: '#990011' }}>2.</span>
                  <span className="font-medium">{incident.summary || 'Elevated behavioral anomaly flagged by deterministic heuristics engine.'}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="font-extrabold" style={{ color: '#990011' }}>3.</span>
                  <span className="font-medium">Enforced Policy Guard: <strong className="uppercase font-bold">{incident.recommendedAction || 'QUARANTINE'}</strong></span>
                </div>
              </div>
            </div>
                  ...
                  {topEvidence.map((e, idx) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        ...
                        <span className="text-[9px] uppercase font-mono px-1.5 py-0.5 rounded font-bold" style={{ background: 'rgba(153,0,17,0.1)', color: '#990011' }}>
                          {e.severity || 'high'}
                        </span>
                      </div>
                      {e.description && (
                        <p className="text-[11px] leading-relaxed line-clamp-2" style={{ color: '#554B49' }}>
                          {e.description}
                        </p>
                      )}
                    </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl text-xs font-bold border transition disabled:opacity-50 hover:bg-white"
                  style={{
          ...
          {/* Footer Navigation */}
          <div className="p-5 border-t flex items-center justify-between gap-3 shrink-0" style={{ background: '#E0D8D4', borderColor: '#C4B5B0' }}>
            {incident.scanId ? (
              <Link
            ...
            {incident._id ? (
              <Link
                ...
                className="px-5 py-2.5 rounded-xl text-xs font-extrabold text-white transition hover:opacity-90 shadow-sm"
